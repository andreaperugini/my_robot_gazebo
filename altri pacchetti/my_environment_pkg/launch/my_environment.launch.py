...
def generate_launch_description():
	...
	world = os.path.join(get_package_share_directory('my_environment_pkg'), 'worlds', world_file_name)		#--headless-rendering da mettere solo per non renderizzare, rendendno piu veloce il training
	gazebo_node = ExecuteProcess(cmd=['gazebo', '--verbose', world,'-s', 'libgazebo_ros_factory.so'], output='screen')
	
	#per non renderizzare usa questo
