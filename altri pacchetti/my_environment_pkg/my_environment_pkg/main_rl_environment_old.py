...
class MyRLEnvironmentNode(Node):
	...
	def __init__ (self): # da togliere save_path
		...
		#codice originale, che non funziona forse perche uno dei 2 topic non ha header
		...
		# Internal storage for synchronization
	...
	def joint_state_callback(self, msg):
		...
		self.try_call_initial_callback()

	def model_state_callback(self, msg):
		self.model_state_msg = msg
		self.try_call_initial_callback()

	def try_call_initial_callback(self):
		if self.joint_state_msg is not None and self.model_state_msg is not None:
			# Se vuoi aggiungere una soglia sul tempo, puoi farlo qui.
			self.initial_callback(self.joint_state_msg, self.model_state_msg)
			self.joint_state_msg = None
			self.model_state_msg = None
